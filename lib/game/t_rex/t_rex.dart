...
class TRex extends PositionComponent with HasGameRef<TRexGame> {
  ...
  // ref to children
  late final WaitingTRex idleDino =
      WaitingTRex(gameRef.spriteCharacter, config);
  late final RunningTRex runningDino =
      RunningTRex(gameRef.spriteCharacter, config);
  late final JumpingTRex jumpingTRex =
      JumpingTRex(gameRef.spriteCharacter, config);
  late final SurprisedTRex surprisedTRex = SurprisedTRex(
    gameRef.spriteCharacter,
    config,
  );
  ...
  double get groundYPos {
    return (gameRef.size.y / 2) - config.height / 2 + 140;
  }
...
class RunningTRex extends TRexStateAnimatedComponent {
  RunningTRex(
    ...
    TRexConfig config,
          ...
          size: Vector2(120.0, 160.0),
          config: config,
          // frames: [Vector2.all(400)]
          frames: [
            Vector2(0.0, 0.0),
            Vector2(120.0, 0.0),
            Vector2(240.0, 0.0),
